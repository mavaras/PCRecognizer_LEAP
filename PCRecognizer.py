...
class leap_listener(Leap.Listener):
    ...
    def on_connect(self, controller):
        ...
        print("conected")
        time.sleep(.5)
        main_window.change_leap_status(True)
    ...
    def on_frame(self, controller):
            ...
            if len(frame.hands) == 2:
                ...
                if self.mouse.active == True:
                    ...
                    if self.deep_mode:
                        ...
                        dist_0_1 = distance_3d(f1.x,f1.y,f1.z, f2.x,f2.y,f2.z)
                        dist_1_2 = distance_3d(f3.x,f3.y,f3.z, f2.x,f2.y,f2.z)
                        print(str(self.mouse.left_clicked)+" "+str(self.mouse.left_pressed))
                        if dist_0_1 < 9.5:
                            print("!")
                ...
                for finger in hand.fingers:
                    ...
                    if finger.type == 1 and self.mouse.active:
                        # cursor movement
        ...
        #cv2.imshow(cv2_window_name, cv_frame_loc)
...
# MAIN ALGORITHM CLASS, which contains all the templates and starts all the process
...
# this controlls the whole application
...
# thread which allows to make "G" + "G" by putting two hands on frame (python .py -algo)
def thread_handler():
    ...
    while not exit:
        if len(listener.frame.hands) == 2 and not listener.capture_frame:
            ...
            pc = Point_cloud("algo", listener.gesture)          # pc containing our new gesture
            pc.draw_on_canvas()                                 # drawing pc on canvas to see the shape

            global points, stroke_id
            points = listener.gesture                           # this allows "F" to work with mouse and hand stroke
            listener.c = 0

            recognize_stroke(points)
...
"""class my_dialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.cvImage = cv2.imread(r"img.jpg")
...
if __name__ == "__main__":
    ...
    # load image test
    #img, height, width = load_image("img.jpg")
    ...
    """pcr.templates[0].point_cloud[0].draw_on_canvas(False)
    pcr.templates[0].point_cloud[1].draw_on_canvas(False)"""
